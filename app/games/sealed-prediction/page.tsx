...
import { completeClientGame,startGame,type GameSessionRef } from "../../../lib/mindplayApi";
...
export default function SealedPrediction(){
 ...
 const begin=()=>{
  ...
  void startGame("sealed-prediction",window.location.pathname)
   .then(started=>setSession({sessionId:started.sessionId,token:started.token}))
 ...
 const choose=(picked:PredictionItem)=>{
  ...
  }else next=candidates.filter(item=>item.id!==picked.id);
  setCandidates(next);setRound(v=>v+1);
  if(next.length===1){
   if(session)void completeClientGame(session,{rounds:round+1}).catch(()=>{});
   setStage("reading");window.setTimeout(()=>setStage("result"),950);
  }
 };

 const restart=()=>{setTarget(createTarget());setCandidates(PREDICTION_ITEMS);setRound(0);setSession(null);setStage("guide")};
 ...
 return <GameFrame title="คำทำนายที่ปิดผนึก" tone="prediction" steps={steps}>
  ...
  {stage==="sealed"&&<section className="stage stage-center stage-enter">
    <div className="sealed-envelope"><Icon name="prediction"/><span/></div><p className="mini-kicker">ปิดผนึกแล้ว</p><h1>คำทำนายถูกซ่อนไว้เรียบร้อย</h1>
    <p className="stage-copy">จากนี้เลือกตามความรู้สึกแรกของคุณ อย่าพยายามเดาว่าเราต้องการให้เลือกอะไร</p><button className="primary-control" onClick={()=>setStage("choice")}>เริ่มตัดสินใจ</button>
  </section>}
  {stage==="choice"&&target&&<section className="stage stage-center stage-enter">
    <p className="mini-kicker">การตัดสินใจ {round+1}</p><h1>แตะหนึ่งสิ่งที่ดึงสายตาคุณที่สุด</h1><p className="stage-copy">เลือกเร็ว ๆ ตามสัญชาตญาณ แล้วปล่อยให้สนามเปลี่ยนไปเอง</p>
  ...
  {stage==="result"&&target&&<ReactionStage analytics={session} icon={target.icon} label={target.label} detail="นี่คือภาพที่ถูกปิดผนึกไว้ก่อนที่คุณจะเริ่มเลือก" onRestart={restart}/>}
 </GameFrame>;
}
