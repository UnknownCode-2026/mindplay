...
export default function CardMind(){
 ...
 const begin=()=>{
  setStage("think");
  void startGame("card-mind",window.location.pathname)
 ...
 const pick=(key:number)=>{
  ...
  if(stage==="lane0")setStage("lane1");
   ...
   setStage("reading");
   if(session)void completeClientGame(session,{rounds:3}).catch(()=>{});
   window.setTimeout(()=>setStage("result"),850);
  }
 };
 const restart=()=>{setSeed(v=>v+71);setKeys([]);setSession(null);setStage("guide")};
 ...
 return <GameFrame title="ไพ่ในความคิด" tone="cards" steps={steps}>
  {stage==="guide"&&<HowToPlay icon="cards" title="ไพ่ในความคิด" intro="เลือกไพ่หนึ่งใบไว้ในหัว แล้วให้ MindPlay ค่อย ๆ ตามรอยโดยที่คุณไม่ต้องแตะไพ่ที่เลือกโดยตรง" steps={steps} onStart={begin}/>}
  ...
  {stage==="result"&&result&&<ReactionStage analytics={session} icon="cards" label={result.rank+" "+suitText} detail="นี่คือไพ่ที่คุณเก็บไว้ในความคิดใช่ไหม?" onRestart={restart}/>}
 </GameFrame>;
}
